...
/**
 * RST-054 / RST-055 — move an order onto another table, merging into whatever is already there.
 *
 * The merge is what makes the "one draft order per table" invariant survive a transfer: the target
 * keeps its identity, guest counts add up, and the kitchen-sent snapshot travels with the lines so
 * nothing is re-fired (RST-056).
 */
export function transferOrder(orderUuid: string, tableId: number): { merged: boolean; orderUuid: string } {
    const state = snapshot();
    const order = state.orders[orderUuid];
    if (!order) return { merged: false, orderUuid };
    if (order.restaurant_table_id === tableId) return { merged: false, orderUuid };

    const target = Object.values(state.orders).find(
        (candidate) =>
            candidate.state === 'draft' &&
            candidate.uuid !== orderUuid &&
            candidate.restaurant_table_id === tableId,
    );

    if (!target) {
        setTable(orderUuid, tableId);
        return { merged: false, orderUuid };
    }

    mergeOrders(orderUuid, target.uuid);
    return { merged: true, orderUuid: target.uuid };
}

/** RST-055 — fold `sourceUuid` into `targetUuid` and delete the source. */
export function mergeOrders(sourceUuid: string, targetUuid: string): void {
    const state = snapshot();
    const source = state.orders[sourceUuid];
    const target = state.orders[targetUuid];
    if (!source || !target || sourceUuid === targetUuid) return;

    const sourceLines = linesOf(state, sourceUuid);
    const targetCourses = coursesOf(state, targetUuid);
    const sourceCourses = coursesOf(state, sourceUuid);

    // Courses are matched by index; a source course with no counterpart is recreated.
    const courseMap = new Map<string, string>();
    for (const course of sourceCourses) {
        const match = targetCourses.find((candidate) => candidate.index === course.index);
        if (match) {
            courseMap.set(course.uuid, match.uuid);
            continue;
        }
        const created: CourseRow = { ...course, uuid: asUuid(generateUuid()), id: null, order_uuid: asUuid(targetUuid), rev: 0 };
        courseMap.set(course.uuid, created.uuid);
        mutate((draft) => {
            draft.courses[created.uuid] = created;
            indexCourse(draft, created);
    ...
    const parentMap = new Map<string, string>();
    const sentSnapshot: Record<string, number> = { ...(target.last_prep_snapshot?.lines ?? {}) };

    for (const line of sourceLines) {
        const sent = source.last_prep_snapshot?.lines[prepKey(line)] ?? 0;
        const newUuid = addLine({
            ...
            comboParentUuid: line.combo_parent_uuid ? (parentMap.get(line.combo_parent_uuid) ?? null) : null,
            courseUuid: line.course_uuid ? (courseMap.get(line.course_uuid) ?? null) : null,
            skipMerge: line.combo_parent_uuid !== null || line.combo_item_id !== null,
            fullProductName: line.full_product_name,
        });
        parentMap.set(line.uuid, newUuid);
        if (sent !== 0) {
            const merged = snapshot().lines[newUuid];
            if (merged) sentSnapshot[prepKey(merged)] = (sentSnapshot[prepKey(merged)] ?? 0) + sent;
        }
    }

    mutate((draft) => {
        const merged = draft.orders[targetUuid];
        if (merged) {
            merged.guest_count += source.guest_count;
            merged.print_count += source.print_count;
            merged.last_prep_snapshot = {
                at: target.last_prep_snapshot?.at ?? nowIso(),
                lines: sentSnapshot,
                noteHash: target.last_prep_snapshot?.noteHash ?? '',
            };
        }
        const dying = draft.orders[sourceUuid];
        if (dying) {
            dying.state = 'cancelled';
            dying.cancelled_at = nowIso();
            dying.cancel_reason = 'merged';
        }
        touch(draft, sourceUuid);
        touch(draft, targetUuid);
        draft.selectedOrderUuid = targetUuid;
    });
