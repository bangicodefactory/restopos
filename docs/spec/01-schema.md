...
pos_configs >─< employees (pos_config_employee: access_level minimal|basic|advanced)
...
| default_role | enum('minimal','cashier','manager') default 'cashier' | fallback when no per-config override |
...
`employees.default_role` / effective register role: `minimal` | `cashier` | `manager`.
